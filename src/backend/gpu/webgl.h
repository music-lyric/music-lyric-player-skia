...
namespace music_lyric_player::backend::gpu {
	 ...
	 * There is no swapchain to present to: a frame ends once its GPU work is flushed, and the browser
	 * composites the canvas by itself after the animation frame callback returns.
	 */
	...
	public:
		 ...
		 * The page owns layout, so only it can know the CSS size and the ratio to scale it by, whereas every
		 * other platform reads both back from the window it was handed.
		 */
	 ...
	 * Each canvas gets a WebGL context and a Skia context of its own, so nothing is shared between
	 * surfaces and a page driving several players pays for several GPU stacks.
